...
#from util import load_node_data
...
'''def load_cora(self):
    adj, features, labels, _, _, _, _ = load_node_data(dataset="cora")
    self.A = adj.numpy()
    self.node_features = features.numpy()
...
class gen_hyper_graph:
    ...
    def load_cora(self):
        ...
        adj = to_dense_adj(data.edge_index)[0]  # Convert to dense adjacency matrix
        features = data.x  # Node feature matrix
        ...
        print(f"CORA dataset loaded. Adjacency matrix shape: {self.A.shape}, Feature matrix shape: {self.node_features.shape}")
    ...
    def run(self):
        """
        This function processes datasets including CORA for motif subgraph generation.
        It handles both pre-defined datasets and the CORA dataset separately.
        """
        for data_name in self.datasets:
            if data_name == "CORA":
                # Load CORA dataset
                self.load_cora()
                print(f"Processing CORA dataset...")

                # Set depth based on CORA graph properties
                self.MAX_DEPTH = 10  # You can tune this if needed
                hyperA, motif2A, motif_label = self.gen_hyperA()

                # Save motif2A and other outputs for CORA
                output_dir = "output/cora"
                os.makedirs(output_dir, exist_ok=True)

                # Save motif2A matrix
                with open(f'{output_dir}/cora_motif2A.txt', 'w') as mf:
                    mf.write(f"{motif2A.shape[0]} {motif2A.shape[1]}\n")
                    for row in motif2A:
                        mf.write(" ".join(map(str, row)) + "\n")
                
                # Save motif labels
                with open(f'{output_dir}/cora_motif_labels.txt', 'w') as lf:
                    for label in motif_label:
                        lf.write(" ".join(map(str, label)) + "\n")

                print(f"CORA dataset processing complete. Results saved in {output_dir}.")

            else:
                # Existing datasets
                dataset_path = os.path.join(self.data_path, data_name)
                label_path = os.path.join(self.label_path, f"{data_name}_label.txt")
                ...
                for file_name in os.listdir(dataset_path):
                    if 'graph' in file_name:
                        ...
                        node_feature_path = os.path.join(dataset_path, f'node_feature_{graph_num}.csv')
                        ...
                        # Save motif2A for current graph
                        output_file = f"dataset/{data_name}/{data_name}_motif2A.txt"
                        with open(output_file, 'a') as mf:
                            mf.write(f"{motif2A.shape[0]} {motif2A.shape[1]}\n")
                            for row in motif2A:
                                mf.write(" ".join(map(str, row)) + "\n")

                        # Collect motif labels
                        motif_labels.append(motif_label)

                print(f"{data_name} dataset processing complete.")
    ...
    def gen_hyperA(self,init_nodeid=0):
        ...
        same_motifs = []
        for i1 in tqdm(range(len(motifs)), desc="Processing Motifs", unit="motif"):
        ...
        for i in tqdm(range(motifs.shape[0]), desc="Processing Motifs", unit="motif"):
            motif=motifs[i]
            for node in motif:
                motif2A[i][node]=1
    ...
    def count_motif2(self):
        ...
        for node_id in tqdm(range(self.A.shape[0])):
            line = self.A[node_id]
