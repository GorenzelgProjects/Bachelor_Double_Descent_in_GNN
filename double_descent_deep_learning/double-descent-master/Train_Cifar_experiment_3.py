...
def main():
    ...
    csv_file = os.path.join(checkpoint_dir, 'training_log_cifar_3.csv')
    ...
    for i in range(15,19):
        k = 1 if i == 0 else i
        for j in range(1, 11):
            checkpoint_path = os.path.join(checkpoint_dir, f'cifar_checkpoint_k{k}_fraction{j}_experiment_3.pth')
            if os.path.exists(checkpoint_path):
                checkpoint = torch.load(checkpoint_path)
                start_i, start_j = i, j
