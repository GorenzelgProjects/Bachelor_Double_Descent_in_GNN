...
def parse_args():
    ...
    parser.add_argument('--use_madreg', action='store_true', help='Use MADReg regularization (overrides config)')
    parser.add_argument('--lambda_reg', type=float, default=None, help='MADReg regularization coefficient (overrides config)')
...
if __name__ == '__main__':
    ...
    try:
        ...
        use_madreg_override = args.use_madreg
        lambda_reg_override = args.lambda_reg
    except KeyError:
        current_folder = os.path.basename(os.getcwd())
        config_file = os.path.join(current_folder, 'configs', 'config_test.json')
        trial_num = 1
        use_madreg_override = False
        lambda_reg_override = None
    ...
    # Parse MADReg settings from config and command line
    use_madreg_config = parse_boolean_string(hyperparams.get("use_madreg", "False"))
    lambda_reg_config = float(hyperparams.get("lambda_reg", 0.1))
    
    # Command line arguments override config settings
    use_madreg = use_madreg_override if use_madreg_override else use_madreg_config
    lambda_reg = lambda_reg_override if lambda_reg_override is not None else lambda_reg_config
    
    print(f"MADReg settings: enabled={use_madreg}, lambda={lambda_reg}")
    
    # Modify output path to include MADReg info if enabled
    if use_madreg:
        # Insert MADReg info before the final underscore
        base_parts = base_output_path.rstrip('_').split('_')
        base_parts.append(f"madreg{lambda_reg}")
        base_output_path = '_'.join(base_parts) + '_'
        print(f"Using MADReg-specific output path: {base_output_path}")
    ...
    print(f"MADReg enabled: {use_madreg}")
    if use_madreg:
    ...
    skip = True if parse_boolean_string(hyperparams.get("skip", "False")) else False
    ppr = True if parse_boolean_string(hyperparams.get("ppr", "False")) else False
    ...
    trainer = Trainer(data=data,
                        ...
                        use_madreg=use_madreg,  # Pass MADReg settings
                        lambda_reg=lambda_reg
                        )
    
    # Train the model with or without MADReg
    if use_madreg:
        print(f"Starting training with MADReg regularization (lambda={lambda_reg})")
        trainer.hyperparameter_train()  # The trainer will handle MADReg internally
    else:
        print("Starting training with standard method")
